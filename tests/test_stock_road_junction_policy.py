...
def test_raw_mixed_t_outside_visible_tolerance_waits_for_transaction():
    native = _native_junction_for_incidents(
        (_incident(0.0, "sil"), _incident(180.0, "sil"), _incident(268.0, "ces"))
    )

    # The raw source arm is two degrees from the measured connector. It is not
    # allowed to place a rigid mesh directly; the connector-relaxation transaction
    # may later propose and validate an aligned version during a complete fit.
    assert native is None


def test_bounded_skewed_mixed_t_is_not_forced_before_approach_alignment():
    incidents = (
        _incident(0.0, "sil"),
        _incident(180.0, "sil"),
        _incident(266.0, "ces"),
...
def test_old_positive_x_style_skew_does_not_bypass_measured_branch_orientation():
    native = _native_junction_for_incidents(
        (
            _incident(84.6, "sil"),
            _incident(264.6, "sil"),
    ...
    # Resistance T Memory LODs put the branch on local -X (270 degrees), not the
    # historical +X assumption. This geometry is therefore not a near-exact T.
    assert native is None
