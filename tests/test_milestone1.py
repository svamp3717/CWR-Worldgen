...
class Milestone1Tests(unittest.TestCase):
    ...
    def test_render_config_registers_procedural_door_buildings(self) -> None:
        ...
        self.assertEqual(config.count('position = "door1_action";'), 2)
        self.assertNotIn('position = "door1_action_inside";', config)
        self.assertNotIn("class OpenDoor1Inside", config)
        self.assertNotIn("class CloseDoor1Inside", config)
        self.assertEqual(config.count('displayName = "Open door";'), 1)
        self.assertEqual(config.count('displayName = "Close door";'), 1)
        self.assertEqual(config.count("radius = 5.0;"), 2)
