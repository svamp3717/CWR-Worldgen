...
def test_skewed_mixed_t_uses_native_paved_gravel_junction():
    # Synthetic representative of a skewed mixed-surface T junction.
    incidents = (
        _junction._Incident(_direction(281.31), "sil", r"O\Road\sil25.p3d"),
        _junction._Incident(_direction(134.69), "sil", r"O\Road\sil25.p3d"),
        _junction._Incident(_direction(37.87), "ces", r"wg_test\i\gravel3.p3d"),
    )
    ...
    assert 16.0 < native.maximum_heading_error_degrees < 17.0
    assert native.maximum_heading_error_degrees <= MAXIMUM_RELAXED_JUNCTION_HEADING_ERROR_DEGREES

    connector_radius = 24.5 * 6.0 / 25.0 * 0.5
    lateral = connector_radius * math.sin(math.radians(native.maximum_heading_error_degrees))
    assert lateral < 1.0
