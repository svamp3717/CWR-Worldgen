...
def _model_only(model_path: str):
    return SimpleNamespace(model_path=model_path)


def _road_piece(model_path: str, length: float, nominal: int):
    return _p._RoadPiece(model_path, length, nominal)


def test_reference_stages_are_active_in_production_stack() -> None:
...
def test_paved_stock_chains_use_full_planar_connector_lengths() -> None:
    paved = (
        _model_only(r"o\road\sil25.p3d"),
        _model_only(r"o\road\sil10 100.p3d"),
    )
    dirt = (
        _model_only(r"o\road\ces25.p3d"),
        _model_only(r"o\road\ces6.p3d"),
    )
...
def test_reference_curve_preference_is_extended_by_kodiak_stage() -> None:
    ...
    assert (
        _curve_usage._MAXIMUM_EXTRA_PIECES
        == _reference.KODIAK_MAXIMUM_EXTRA_CURVE_PIECES
    )
    assert math.isclose(
...
def test_paved_junction_window_overlaps_measured_footprint(monkeypatch) -> None:
    ...
    pieces = (
        _road_piece(r"o\road\sil25.p3d", 25.0, 25),
        _road_piece(r"o\road\sil12.p3d", 12.5, 12),
        _road_piece(r"o\road\sil6.p3d", 6.25, 6),
    )
...
def test_reference_junction_overlap_does_not_change_ces(monkeypatch) -> None:
    measure = _p._PolylineMeasure.create(((0.0, 0.0), (0.0, 100.0)))
    pieces = (_road_piece(r"o\road\ces25.p3d", 25.0, 25),)
