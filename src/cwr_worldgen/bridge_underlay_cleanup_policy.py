...
"""Remove ordinary road-piece chains that survive underneath generated bridges."""
...
def _bridge_spans(dataset, projection, elevations, spec) -> tuple[_BridgeSpan, ...]:
    ...
    module_length = (
        ...
        if procedural
        else _osm.NOGOVA_BRIDGE_MODULE_LENGTH_METRES
    )
    ...
    for feature, raw_points in zip(dataset.roads, projected):
        ...
        if _osm.road_bridge_crosses_ditch_only(feature, dataset, projection):
            continue
        chunks = _osm._bridge_module_chunks(points, module_length)
        if not chunks or not _feature_needs_bridge(feature, points, elevations, spec):
            continue
        required = 1 if procedural else len(chunks)
...
def _road_object_under_bridge(obj, spans: tuple[_BridgeSpan, ...]) -> bool:
    ...
    # roads and generated gravel, while excluding junction caps and bridge P3Ds.
    ...
    for span in spans:
        ...
        endpoint_keep = min(_ENDPOINT_KEEP_METRES, total * 0.10)
        if along <= endpoint_keep or along >= total - endpoint_keep:
            continue
        # The duplicate chain is fitted on the source road centreline. Keep this
        # corridor deliberately narrow so nearby parallel roads and at-grade
        # crossings are not mistaken for bridge underlays.
...
def _fit(
    ...
    *,
    ...
    if removed and progress_callback is not None:
        progress_callback(
            99,
            f"Removed {removed:,} ordinary road piece(s) underneath bridge spans",
        )
    return cleaned
