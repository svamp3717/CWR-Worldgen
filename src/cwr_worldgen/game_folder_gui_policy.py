...
PREFERRED_GUI_WIDTH = 1240
PREFERRED_GUI_HEIGHT = 920
GUI_SCREEN_MARGIN_X = 80
GUI_SCREEN_MARGIN_Y = 100
_INSTALLED = False


def preferred_gui_window_size(screen_width: int, screen_height: int) -> tuple[int, int]:
    """Return a roomy window size without overflowing smaller displays."""

    available_width = max(760, int(screen_width) - GUI_SCREEN_MARGIN_X)
    available_height = max(620, int(screen_height) - GUI_SCREEN_MARGIN_Y)
    return (
        min(PREFERRED_GUI_WIDTH, available_width),
        min(PREFERRED_GUI_HEIGHT, available_height),
    )
...
def _install_into_gui(gui: Any) -> None:
    ...
    original_init = gui.WorldgenGui.__init__
    ...
    def init(self, *args, **kwargs) -> None:
        ...
        try:
            width, height = preferred_gui_window_size(
                self.winfo_screenwidth(), self.winfo_screenheight()
            )
            self.geometry(f"{width}x{height}")
            self.minsize(min(960, width), min(720, height))
        except Exception:
            # Window sizing is convenience only. Never make GUI startup depend on
            # a window manager reporting sane screen dimensions.
            pass
    ...
    def build_world_page(self) -> None:
        ...
        # Add the persistent game installation immediately after World identity.
        # With the roomier default window this remains visible without requiring
        # a scrollbar on ordinary 1080p desktops.
        ...
        gui.ttk.Label(
            ...
            style="Hint.TLabel",
            wraplength=760,
        ).grid(row=1, column=1, columnspan=2, sticky="w", pady=(0, 2))
