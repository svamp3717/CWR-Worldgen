...
from .procedural_infrastructure import (
    is_generated_gravel_curve_model,
...
def _stock_road_model_for_tags(spec, tags):
    ...
    if (
        is_generated_gravel_road_model(str(model))
        or is_generated_gravel_curve_model(str(model))
        or is_generated_gravel_junction_model(str(model))
    ):
...
def _stock_native_t_dispatch(incidents):
    ...
    # Backward compatibility for callers that construct legacy generated-gravel
    # incidents explicitly.  Normal production road selection above never does.
...
def _generated_road_model(model_path: str) -> bool:
    ...
    return (
        is_generated_gravel_road_model(path)
        or is_generated_gravel_curve_model(path)
        or is_generated_gravel_junction_model(path)
...
def _fit(
    ...
    *,
    ...
    if forbidden:
        sample = ", ".join(
            sorted({str(obj.model_path) for obj in forbidden})[:6]
        )
        raise ValueError(
