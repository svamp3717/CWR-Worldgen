...
def _bridge_lods(key: InfrastructureModelKey, texture: str) -> tuple[_Lod, ...]:
    ...
    points: list[tuple[float, float, float]] = []
    faces: list[_Face] = []
    _append_box(points, faces, x0=-half_w, x1=half_w, y0=deck_bottom, y1=deck_top, z0=-half_l, z1=half_l, texture=texture)
    rail_x = half_w + 0.08
    for sign in (-1.0, 1.0):
        x0 = sign * rail_x - 0.08
        x1 = sign * rail_x + 0.08
        _append_box(points, faces, x0=min(x0, x1), x1=max(x0, x1), y0=deck_top, y1=1.28, z0=-half_l, z1=half_l, texture=texture)
    # Keep end abutments shallow; their purpose is visual closure, not terrain
    # grounding. The actual WRP object origin controls bridge height.
    if key.subtype in {"start", "single"}:
        _append_box(points, faces, x0=-half_w - 0.35, x1=half_w + 0.35, y0=-0.55, y1=deck_top, z0=-half_l, z1=-half_l + 0.50, texture=texture)
    if key.subtype in {"end", "single"}:
        _append_box(points, faces, x0=-half_w - 0.35, x1=half_w + 0.35, y0=-0.55, y1=deck_top, z0=half_l - 0.50, z1=half_l, texture=texture)
    # The bridge structure itself must not be another class=road object. The
    # fitted stock road chain underneath already owns CWA's road-network/map
    # semantics. Stacking a second road-class P3D several metres above it makes
    # Poseidon's terrain/road clipping path treat the bridge deck like terrain-
    # fitted road geometry, which can make modules sink or vanish in game even
    # though their WRP transforms are correct. Keep this model as an ordinary
    # static structure with a Roadway LOD instead.
    visual = _Lod(
        tuple(points), ((0.0, 1.0, 0.0),), tuple(faces), _VISUAL_LOD,
        properties=(("autocenter", "0"),),
    )
    geometry_boxes = ((-half_w, half_w, deck_bottom, deck_top, -half_l, half_l),)
    raw_geometry = _geometry_from_boxes(geometry_boxes)
    geometry = _Lod(
    ...
    roadway_points = ((-half_w, roadway_y, -half_l), (half_w, roadway_y, -half_l), (half_w, roadway_y, half_l), (-half_w, roadway_y, half_l))
    roadway = _Lod(roadway_points, ((0.0, 1.0, 0.0),), (_quad("", 0, 3, 2, 1),), _ROADWAY_LOD)
    land = _Lod(((-half_w, deck_bottom, -half_l), (half_w, deck_bottom, -half_l), (-half_w, deck_bottom, half_l), (half_w, deck_bottom, half_l)), (), (), _LAND_CONTACT_LOD)
    ...
    def write_assets(self, source_dir: Path, catalogue_path: Path) -> InfrastructureAssetResult:
        ...
        for key in sorted(self._usage):
            ...
            model_cache_version = (
                ...
                if key.kind == "road"
                else "procedural-infrastructure-model-v16-static-bridge-roadway"
                if key.kind == "bridge"
