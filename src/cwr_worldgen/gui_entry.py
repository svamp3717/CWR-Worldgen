...
function render(){{list.innerHTML='';let shown=0;for(const i of issues){{const show=visible(i),marker=markerEls.get(i.id);if(marker)marker.style.display=show?'':'none';if(!show)continue;shown++;const d=document.createElement('div');d.className=`issue ${{i.severity}}`;d.dataset.row=i.id;d.innerHTML=`<div class="issuehead"><span>${{esc(i.id)}} · ${{esc(i.severity.toUpperCase())}}</span><span>${{i.score.toFixed(1)}}</span></div><div class="category">${{esc(i.category)}}</div><div class="coords">${{i.x.toFixed(2)}}, ${{i.z.toFixed(2)}} · objects ${{esc(i.objects.join(', '))}}</div><div>${{esc(i.message)}}</div><div class="coords">player setPos [${{i.x.toFixed(3)}}, ${{i.z.toFixed(3)}}, 0]</div>`;d.onclick=()=>selectIssue(i);list.appendChild(d)}}document.getElementById('stats').innerHTML=`<span class="stat">${{summary.roads}} road objects</span><span class="stat">${{summary.issues}} findings</span><span class="stat">${{shown}} shown</span>`}}
